...
use App\Services\Api\Contracts\ApiInterface;
...
class ApiResponse implements ApiInterface
{
     ...
     * @return JsonResponse
     */
    public static function dataOk($message = null, $data = [], ...$extraData)
    {
        if (empty($data) || (($data instanceof Collection) && $data->isEmpty())) {
            return static::response(static::HTTP_FORBIDDEN, __('No data found'));
        }

        return static::ok($message, $data);
    }
    ...
    public static function session_expired($message = null, $data = [])
    {
        if (is_null($message)) {
            $message = config('api.messages.session_expired');
        }

        return static::response(static::HTTP_SESSION_EXPIRED, $message, $data);
    }
